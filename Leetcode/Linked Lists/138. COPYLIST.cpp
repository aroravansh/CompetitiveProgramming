...
[val, random_index] where: val: an integer representing Node.val
...
public:
    Node* copyRandomList(Node* head) {
        ...
        while(prev)
        {
            prev->next->random = prev->random?prev->random->next:prev->random;
            prev = prev->next->next;
        }
        prev = head;
        while(prev)
        {
            forw = prev->next->next;
            prev->next->next = forw?forw->next:forw;
            prev->next = forw;
            prev = forw;
        }
        return newhead;
